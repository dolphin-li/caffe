...
namespace caffe {
		...
		const int M, const int N, const Dtype* A, const Dtype* B, Dtype* C)
	{
		...
		{
			for(int x=0; x<N; x++)
				caffe_cpu_scale(M, B[x], A+x*M, C);
		}
		else if(mode == CUBLAS_SIDE_LEFT)
		{
			for(int x=0; x<N; x++)
				caffe_mul(M, B, A+x*M, C);
		}
		else
			LOG(ERROR) << "un-recognized parameter:" << mode;
	}

	template 
	void caffe_cpu_dgmm(const cublasSideMode_t mode,
		const int M, const int N, const double* A, const double* B, double* C);
	template 
	void caffe_cpu_dgmm(const cublasSideMode_t mode,
		const int M, const int N, const float* A, const float* B, float* C);

	template <>
	void caffe_gpu_dgmm(const cublasSideMode_t mode,
		const int M, const int N, const float* A, const float* B, float* C)
	{
		CUBLAS_CHECK(cublasSdgmm(Caffe::cublas_handle(), mode, M, N, A, M, B, 1, C, M));
	}
	...
	void caffe_gpu_dgmm(const cublasSideMode_t mode,
		const int M, const int N, const double* A, const double* B, double* C)
	{
		CUBLAS_CHECK(cublasDdgmm(Caffe::cublas_handle(), mode, M, N, A, M, B, 1, C, M));
	}
